...
namespace :svg_generator do
  ...
  task :create_theme, :response do |t, args|
    ...
    if new_required_theme
      ...
      new_text = new_text.sub("#{test_constant} = :#{test_symbol}", "T_#{theme_name.upcase}_THEME = :#{new_theme_constant}Theme\n  #{test_constant} = :#{test_symbol}")
      new_text = new_text.sub("#{test_constant},", "#{test_constant}, T_#{theme_name.upcase}_THEME")
      ...
      Dir[dir + "/*.*"].each do |file|
        ...
        new_text = new_text.sub("test theme", "#{theme_name.gsub('_', ' ')} theme")
      ...
      text = File.read(dir)
      new_text = text.sub("TestTheme", "#{theme_name.split('_').collect(&:capitalize).join}Theme")

      new_text = new_text.sub('theme', "#{theme_name.gsub('_', ' ')} theme")
